...
package nochump.util.zip; 
	...
	class ZipFile extends EventDispatcher {
		...
		var buf:ByteArray; // data from which zip entries are read.
		var entryList:Array<Dynamic>;
		var entryTable:Dictionary;
		var locOffsetTable:Dictionary;
		...
		function onZipEntryParsed(event:ZipEvent):Void {
			dispatchEvent( new ZipEvent(ZipEvent.ENTRY_PARSED, false, false, event.entry) );
		}

		function onZipEntryProgress(event:ProgressEvent):Void {
