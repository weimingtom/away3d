...
class DirectionalLight extends LightPrimitive  {
	 ...
	 */
	public var diffuseTransform:HashMap<Object3D, Matrix3D>;
	/**
	 * Transform dictionary for the specular lightmap used by shading materials.
	 */
	public var specularTransform:HashMap<Object3D, HashMap<View3D, Matrix3D>>;
	/**
	 ...
	 */
	public var colorMatrixTransform:HashMap<Object3D, BitmapFilter>;
	/**
	 * Colormatrix transform used in DOT3 materials for resolving normal values in the normal map.
	 */
	public var normalMatrixTransform:HashMap<Object3D, BitmapFilter>;
	...
	public function clearTransform():Void {
		...
		diffuseTransform = new HashMap<Object3D, Matrix3D>();
		specularTransform = new HashMap<Object3D, HashMap<View3D, Matrix3D>>();
		colorMatrixTransform = new HashMap<Object3D, BitmapFilter>();
		normalMatrixTransform = new HashMap<Object3D, BitmapFilter>();
	}
	...
	public function setDiffuseTransform(source:Object3D):Void {
		...
		if (!diffuseTransform.contains(source)) {
			diffuseTransform.put(source, new Matrix3D());
		}
		diffuseTransform.get(source).multiply3x3(transform, source.sceneTransform);
		diffuseTransform.get(source).normalize(diffuseTransform.get(source));
	}
	...
	public function setSpecularTransform(source:Object3D, view:View3D):Void {
		...
		if (!specularTransform.contains(source)) {
			specularTransform.put(source, new HashMap<View3D, Matrix3D>());
		}
		
		if (!specularTransform.get(source).contains(view)) {
			specularTransform.get(source).put(view, new Matrix3D());
		}
		specularTransform.get(source).get(view).multiply3x3(halfTransform, source.sceneTransform);
		specularTransform.get(source).get(view).normalize(specularTransform.get(source).get(view));
	}
	...
	public function setColorMatrixTransform(source:Object3D):Void {
		...
		colorMatrixTransform.put(source, _colorMatrix.clone());
	}
	...
	public function setNormalMatrixTransform(source:Object3D):Void {
		...
		_szx = diffuseTransform.get(source).szx;
		_szy = diffuseTransform.get(source).szy;
		_szz = diffuseTransform.get(source).szz;
		_normalMatrix.matrix = [_szx, 0, 0, 0, 127 - _szx * 127, 0, -_szy, 0, 0, 127 + _szy * 127, 0, 0, _szz, 0, 127 - _szz * 127, 0, 0, 0, 1, 0];
		normalMatrixTransform.put(source, _normalMatrix.clone());
	}
	...
	public function new () {
		...
		this.colorMatrixTransform = new HashMap<Object3D, BitmapFilter>();
		this.normalMatrixTransform = new HashMap<Object3D, BitmapFilter>();
