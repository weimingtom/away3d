...
class Number3D  {
	...
	private static inline var MathPI:Float = Math.PI;
	 ...
	 */
	public function new(?x:Float=0, ?y:Float=0, ?z:Float=0, ?n:Bool=false) {
	 ...
	 */
	public function matrix2euler(m:Matrix3D, ?scaleX:Float=1, ?scaleY:Float=1, ?scaleZ:Float=1):Void {
		...
		var cy:Float = Math.sqrt(m1.sxx * m1.sxx + m1.syx * m1.syx);
		// rot.y = Math<T>::atan2 (-N[0][2], cy);
		y = Math.atan2(-m1.szx, cy);
