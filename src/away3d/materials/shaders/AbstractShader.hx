...
class AbstractShader extends EventDispatcher, implements ILayerMaterial {
	...
	private function getLightingShape(layer:Sprite, light:LightPrimitive):Shape {
		...
		_session = _source.session;
		if (_shapeDictionary[cast _session] == null) {
			_shapeDictionary[cast _session] = new Dictionary();
		}
		//check to see if source shape exists
		if ((_shape = _shapeDictionary[cast _session][cast light]) == null) {
			layer.addChild(_shape = _shapeDictionary[cast _session][cast light] = new Shape());
		}
		return _shape;
	}
	...
	public function new(?init:Dynamic=null) {
		...
		this._spriteDictionary = new Dictionary();
