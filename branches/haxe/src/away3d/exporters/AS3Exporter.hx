...
class AS3Exporter  {
	...
	private var geonums:HashMap<Element, Int>;
	private var facenumsHashMap<Element, Int>;
	...
	private function readVertexAnimation(obj:Mesh, id:String):Void {
		...
		var verticesorder:Array<Int> = fr.getIndexes(obj.vertices);
		...
		if (Std.is(object3d, ObjectContainer3D)) {
			...
			if (containerid != -1) {
				...
				if (obj.materialLibrary != null) {
					...
					for (materialData in obj.materialLibrary.iterator()) {
						if (materialData != null) {
							...
							for (__i in 0...materialData.elements.length) {
								...
								if (element != null) {
									if (geonums.contains(element) && facenums.contains(element)) {
										materialString += "\t\t\tmElements_" + mcount + ".push(geos[" + geonums.get(element) + "].geometry.faces[" + facenums.get(element) + "]);\n";
									}
								}
							}
	...
	function new(object3d:Object3D, classname:String, ?packagename:String="") {
		...
		this.objcount = 0;
		this.geocount = 0;
		this.geonums = new HashMap<Element, Int>();
		this.facenums = new HashMap<Element, Int>();
		this.indV = 0;
		this.indVt = 0;
		this.indF = 0;
		this.geos = [];
