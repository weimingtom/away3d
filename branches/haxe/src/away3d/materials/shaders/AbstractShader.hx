 ...
 */
class AbstractShader extends HashableEventDispatcher, implements ILayerMaterial {
	...
	/** @private */
	public var _simpleShapeDictionary:HashMap<AbstractRenderSession, Shape>;
	public var _lightingShapeDictionary:HashMap<AbstractRenderSession, HashMap<LightPrimitive, Shape>>;
	/** @private */
	...
	public function clearShapeDictionary():Void {
		...
		for (_shape in _simpleShapeDictionary.iterator()) {
			if (_shape != null) {
	...
	public function clearLightingShapeDictionary():Void {
		var lightMap:HashMap<LightPrimitive, Shape>;
		for (lightMap in _lightingShapeDictionary.iterator()) {
			if (lightMap != null) {
				
				for (_shape in lightMap.iterator()) {
					if (_shape != null) {
						_shape.graphics.clear();
					}
				}
	...
	private function getShape(layer:Sprite):Shape {
		...
		if ((_shape = _simpleShapeDictionary.get(_session)) == null) {
			_simpleShapeDictionary.put(_session, new Shape());
			layer.addChild(_shape = _simpleShapeDictionary.get(_session));
		}
		return _shape;
	}
	...
	private function getLightingShape(layer:Sprite, light:LightPrimitive):Shape {
		...
		_session = _source.session;
		if (_lightingShapeDictionary.get(_session) == null) {
			_lightingShapeDictionary.put(_session, new HashMap<LightPrimitive, Shape>());
		}
		//check to see if source shape exists
		if ((_shape = _lightingShapeDictionary.get(_session).get(light)) == null) {
			_lightingShapeDictionary.get(_session).put(light, new Shape());
			layer.addChild(_shape = _lightingShapeDictionary.get(_session).get(light));
		}
		return _shape;
	}
	...
	public function new(?init:Dynamic=null) {
		...
		this._simpleShapeDictionary = new HashMap<AbstractRenderSession, Shape>();
		this._lightingShapeDictionary = new HashMap<AbstractRenderSession, HashMap<LightPrimitive, Shape>>();
		this._s = new Shape();
	...
	public function getFaceMaterialVO(faceVO:FaceVO, ?source:Object3D=null, ?view:View3D=null):FaceMaterialVO {
		...
		_faceDictionary.put(faceVO, new FaceMaterialVO());
		return _faceDictionary.get(faceVO);
	}
