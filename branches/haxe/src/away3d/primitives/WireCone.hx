package away3d.primitives;
    ...
    */ 
    class WireCone extends AbstractWirePrimitive {
        ...
        function buildWireCone(radius:Float, height:Float, segmentsW:Int, segmentsH:Int, yUp:Bool):Void
        {
            var i:Int;
            var j:Int;
            ...
            { 
                var z:Int = -height + 2 * height * (j-1) / (segmentsH-1);

                grid[j] = new Array(segmentsW);
                for (i in 0...segmentsW) 
                { 
                    var verangle:Int = 2 * i / segmentsW * Math.PI;
                    var ringradius:Int = radius * (segmentsH-j)/(segmentsH-1);
