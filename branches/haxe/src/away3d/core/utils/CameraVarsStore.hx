...
class CameraVarsStore  {
	...
	public function createVertexClassificationDictionary(source:Object3D):Dictionary {
		...
		if ((_vertexClassificationDictionary = _sourceDictionary[cast source]) == null) {
			_vertexClassificationDictionary = _sourceDictionary[cast source] = new Dictionary();
		}
	...
	public function createVertexClassification(vertex:Vertex):VertexClassification {
		...
		if (((_vc = _vertexClassificationDictionary[cast vertex]) != null)) {
			return _vc;
		}
		if ((_vcStore.length > 0)) {
			_vc = _vertexClassificationDictionary[cast vertex] = _vcStore.pop();
		} else {
			_vc = _vertexClassificationDictionary[cast vertex] = new VertexClassification();
		}
		_vc.vertex = vertex;
		_vc.plane = null;
		return _vc;
	...
	public function createViewTransform(node:Object3D):Matrix3D {
		...
		if ((_vtStore.length > 0)) {
			_vtActive.push(_vt = viewTransformDictionary[cast node] = _vtStore.pop());
		} else {
			_vtActive.push(_vt = viewTransformDictionary[cast node] = new Matrix3D());
		}
		return _vt;
	}
	...
	public function createFrustum(node:Object3D):Frustum {
		...
		if ((_frStore.length > 0)) {
			_frActive.push(_frustum = frustumDictionary[cast node] = _frStore.pop());
		} else {
			_frActive.push(_frustum = frustumDictionary[cast node] = new Frustum());
		}
		return _frustum;
	}
	...
	public function createVertex(x:Float, y:Float, z:Float):Vertex {
		...
		if ((_vStore.length > 0)) {
			...
			_vActive.push(_vertex = new Vertex());
		}
		return _vertex;
	}
	...
	public function createUV(u:Float, v:Float, session:AbstractRenderSession):UV {
		...
		if ((_uvArray = _uvDictionary[cast session]) == null) {
			_uvArray = _uvDictionary[cast session] = [];
		}
	...
	public function reset():Void {
		...
		for (_object in __keys) {
			...
			if (_source.session && _source.session.updated) {
				...
				for (_v in __keys) {
					_vcStore.push(_sourceDictionary[cast _source][cast _v]);
					_sourceDictionary[cast _source][cast _v] = null;
		...
		nodeClassificationDictionary = new Dictionary();
		...
		for (_object in __keys) {
			...
			if (_session.updated) {
				_uvStore = _uvStore.concat(cast(_uvDictionary[cast _session], Array<Dynamic>));
				_uvDictionary[cast _session] = [];
			}
	...
	public function new () {
		...
		this._uvDictionary = new Dictionary();
		this._uvStore = new Array<Dynamic>();
		this._fActive = new Array<Dynamic>();
		this._fStore = new Array<Dynamic>();
		this.viewTransformDictionary = new Dictionary();
		this.frustumDictionary = new Dictionary();
