...
class PrimitiveTraverser extends Traverser  {
	...
	public function setView(val:View3D):View3D {
		...
		_projectorDictionary[cast ProjectorType.CONVEX_BLOCK] = _view._convexBlockProjector;
		_projectorDictionary[cast ProjectorType.DIR_SPRITE] = _view._dirSpriteProjector;
		_projectorDictionary[cast ProjectorType.DOF_SPRITE] = _view._dofSpriteProjector;
		_projectorDictionary[cast ProjectorType.MESH] = _view._meshProjector;
		_projectorDictionary[cast ProjectorType.MOVIE_CLIP_SPRITE] = _view._movieClipSpriteProjector;
		_projectorDictionary[cast ProjectorType.OBJECT_CONTAINER] = _view._objectContainerProjector;
		_projectorDictionary[cast ProjectorType.SPRITE] = _view._spriteProjector;
		return val;
	}
	...
	public function new() {
		// autogenerated
		super();
		this._projectorDictionary = new Dictionary();
	...
	public override function match(node:Object3D):Bool {
		...
		if (!node.visible || (_clipping.objectCulling && _cameraVarsStore.nodeClassificationDictionary[cast node] == null)) {
			return false;
		}
	...
	public override function apply(node:Object3D):Void {
		...
		if (node.session.updated) {
			_viewTransform = _cameraVarsStore.viewTransformDictionary[cast node];
			_consumer = node.session.getConsumer(_view);
			if (node.projectorType) {
				(cast(_projectorDictionary[cast node.projectorType], IPrimitiveProvider)).primitives(node, _viewTransform, _consumer);
			}
			if (node.debugbb && node.debugBoundingBox.visible) {
				node.debugBoundingBox._session = node.session;
				if (_clipping.objectCulling) {
					_cameraVarsStore.frustumDictionary[cast node.debugBoundingBox] = _cameraVarsStore.frustumDictionary[cast node];
					_nodeClassification = _cameraVarsStore.nodeClassificationDictionary[cast node];
			...
			if (node.debugbs && node.debugBoundingSphere.visible) {
				...
				if (_clipping.objectCulling) {
					_cameraVarsStore.frustumDictionary[cast node.debugBoundingSphere] = _cameraVarsStore.frustumDictionary[cast node];
					_nodeClassification = _cameraVarsStore.nodeClassificationDictionary[cast node];
					if (_nodeClassification == Frustum.INTERSECT) {
			...
			if (Std.is(node, ILightProvider)) {
				...
				if (_light.debug) {
					...
					if (_clipping.objectCulling) {
						_cameraVarsStore.frustumDictionary[cast _light.debugPrimitive] = _cameraVarsStore.frustumDictionary[cast _light];
					}
