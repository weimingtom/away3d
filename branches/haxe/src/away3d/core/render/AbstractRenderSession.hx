...
class AbstractRenderSession extends EventDispatcher  {
	...
	public function notifySessionUpdate():Void {
		...
		if (_sessionupdated == null) {
			_sessionupdated = new SessionEvent();
		}
	...
	private function getDOStore(view:View3D):Array<Dynamic> {
		...
		if (_doStores[cast view] == null) {
			return _doStores[cast view] = new Array<Dynamic>();
		}
		return _doStores[cast view];
	}

	private function getDOActive(view:View3D):Array<Dynamic> {
		
		if (_doActives[cast view] == null) {
			return _doActives[cast view] = new Array<Dynamic>();
		}
		return _doActives[cast view];
	}
	...
	public function getConsumer(view:View3D):IPrimitiveConsumer {
		...
		if ((_renderers[cast view] != null)) {
			return _renderers[cast view];
		}
		if ((_renderer != null)) {
			return _renderers[cast view] = _renderer.clone();
		}
		if ((parent != null)) {
			return _renderers[cast view] = parent.getConsumer(view).clone();
		}
		return _renderers[cast view] = (cast(view.session.renderer, IPrimitiveConsumer)).clone();
	}
	...
	public function clear(view:View3D):Void {
		...
		updated = view.updated || view.forceUpdate || view.scene.updatedSessions[cast this];
		for (__i in 0...sessions.length) {
			_session = sessions[__i];
