	...
	public class HingeJoint extends PhysicsController
	{
			...
			hingeBckAngle:Number, sidewaysSlack:Number, damping:Number)
		{
			...
			if (hingeFwdAngle <= MAX_HINGE_ANGLE_LIMIT)
			{
				...
				JMatrix3D.getMultiplyVector(JMatrix3D.getRotationMatrix(_hingeAxis.x, _hingeAxis.y, _hingeAxis.z, -angleToMiddle * Math.PI / 180), hingeRelAnchorPos1);
		...
		override public function updateController(dt:Number):void
		{
			...
			if (_extraTorque != 0)
			{
				...
				JMatrix3D.getMultiplyVector(_body0.currentState.orientation, torque1);
