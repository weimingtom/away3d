...
package jiglib.collision
{
	...
	public class CollDetectCapsuleCapsule extends CollDetectFunctor
	{
		...
		override public function collDetect(info:CollDetectInfo, collArr:Vector.<CollisionInfo>):void
		{
			...
			var oldSeg0:JSegment = new JSegment(capsule0.getBottomPos(capsule0.oldState), JNumber3D.getScaleVector(capsule0.oldState.orientation__getCols()[1], capsule0.length));
			var newSeg0:JSegment = new JSegment(capsule0.getBottomPos(capsule0.currentState), JNumber3D.getScaleVector(capsule0.currentState.orientation__getCols()[1], capsule0.length));
			var oldSeg1:JSegment = new JSegment(capsule1.getBottomPos(capsule1.oldState), JNumber3D.getScaleVector(capsule1.oldState.orientation__getCols()[1], capsule1.length));
			var newSeg1:JSegment = new JSegment(capsule1.getBottomPos(capsule1.currentState), JNumber3D.getScaleVector(capsule1.currentState.orientation__getCols()[1], capsule1.length));
			...
			if (Math.min(oldDistSq, newDistSq) < Math.pow(radSum + JConfig.collToll, 2))
			{
				...
				else
				{
					...
					JMatrix3D.getMultiplyVector(JMatrix3D.getRotationMatrix(0, 0, 1, 360 * Math.random()), delta);
				}
			...
			oldSeg0 = new JSegment(capsule0.getEndPos(capsule0.oldState), JNumber3D.getScaleVector(capsule0.oldState.orientation__getCols()[1], capsule0.length));
			newSeg0 = new JSegment(capsule0.getEndPos(capsule0.currentState), JNumber3D.getScaleVector(capsule0.currentState.orientation__getCols()[1], capsule0.length));
			oldSeg1 = new JSegment(capsule1.getEndPos(capsule1.oldState), JNumber3D.getScaleVector(capsule1.oldState.orientation__getCols()[1], capsule1.length));
			newSeg1 = new JSegment(capsule1.getEndPos(capsule1.currentState), JNumber3D.getScaleVector(capsule1.currentState.orientation__getCols()[1], capsule1.length));
