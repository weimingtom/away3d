	...
	public class ExKeyBoardControl extends PhysicsTemplate
	{
		...
		private function init3D():void
		{
			...
			for (i = 0; i < 6; i++)
			{
				...
				boxBody[i].moveTo(new Vector3D(0, 500 + (100 * i + 100), 0));
			}
		}
		...
		override protected function onPreRender():void
		{
			if (keyLeft)
			{
				_ball.addWorldForce(new Vector3D(100, 0, 0), _ball.currentState.position);
			}
			if (keyRight)
			{
				_ball.addWorldForce(new Vector3D(-100, 0, 0), _ball.currentState.position);
			...
			if (keyForward)
			{
				_ball.addWorldForce(new Vector3D(0, 0, 100), _ball.currentState.position);
			}
			if (keyReverse)
			{
				_ball.addWorldForce(new Vector3D(0, 0, -100), _ball.currentState.position);
			...
			if (keyUp)
			{
				_ball.addWorldForce(new Vector3D(0, 100, 0), _ball.currentState.position);
			}
