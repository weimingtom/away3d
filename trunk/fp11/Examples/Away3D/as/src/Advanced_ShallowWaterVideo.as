package
{
	...
	public class Advanced_ShallowWaterVideo extends Sprite
	{
		//signature swf
		...
		[Embed(source="../embeds/assets.swf", symbol="VideoMask")]
		private var VideoMask : Class;
		...
		private var _showingLiquidVideo:Boolean;
		...
		private var mtx:Matrix = new Matrix(-1, 0, 0, -1, 320-60, 220);
		private var blurFilter:BlurFilter = new BlurFilter( 4, 4, 3 );
		private var destPoint:Point = new Point();
		private var videoBitmapData:BitmapData;
		private var bitmap:Bitmap;
		private var count:uint;
		private var videoVector:Vector.<uint>;
		private var videoTimer:Timer;
		
		/**
		 * Constructor
		 */
		public function Advanced_ShallowWaterVideo()
		{
			init();
		}
		...
		private function initEngine():void
		{
			...
			webcamera.setQuality(bandwidth, quality);
			webcamera.setMode(320,240,15,true); // setMode(videoWidth, videoHeight, video fps, favor area)
			...
			videoBitmapData = new BitmapData(198,198, false, 0);
			bitmap = new Bitmap(videoBitmapData);
			addChild(bitmap);
			
			videoTimer = new Timer(100);
			videoTimer.addEventListener(TimerEvent.TIMER, onVideoTimer);
		}
		...
		private function initGUI() : void
		{
			...
			_gui.addToggle("toggleLiquidVideo", {label:"video"});
		...
		private function onEnterFrame(event:Event):void
		{
			...
			if (_showingLiquidVideo) {
				if (count < 64) count++
				_fluidDisturb.releaseMemoryDisturbances();
				videoBitmapData.draw(video, mtx);
				/*
				videoVector = videoBitmapData.getVector(videoBitmapData.rect);
				
				var i:uint, j:uint, p:uint, p2:uint, p3:uint;
				var width:uint = videoBitmapData.width - 1;
				var height:uint = videoBitmapData.height - 1;
				
				for(i = 0; i < width - 1; ++i)
				{
					for(j = 0; j < height - 1; ++j)
					{
						p = videoVector[uint(i + j*width)] & 0x0000FF;
						
						p2 = videoVector[uint(i + j*width + 1)] & 0x0000FF;
						
						p3 = videoVector[uint(i + j*width + width)] & 0x0000FF;
						
						if( Math.min(255, 2*Math.abs(p - p2)) + Math.min(255, 2*Math.abs(p - p3)) > 50 )
							videoVector[uint(i + j*width)] = 0xFFFFFF;
						else
							videoVector[uint(i + j*width)] = 0x0;
					}
				}
				videoBitmapData.setVector(videoBitmapData.rect, videoVector);
				*/
				videoBitmapData.applyFilter(videoBitmapData, videoBitmapData.rect, destPoint, blurFilter);
				videoBitmapData.threshold(videoBitmapData, videoBitmapData.rect, destPoint, "<", 0x330000, 0x0, 0xFF0000, true);
				_fluidDisturb.disturbBitmapMemory(0.5, 0.5, -40*count, videoBitmapData, -1, 0.01);
		...
		private function onVideoTimer(event:TimerEvent):void
		{
		...
		public function set toggleLiquidVideo(value : Boolean) : void
		{
			_showingLiquidVideo = value;
			if (_showingLiquidVideo) {
				//videoBitmapData.draw(video);
				//_fluidDisturb.disturbBitmapMemory(0.5, 0.5, -10, videoBitmapData, -1, 0.01);
				videoTimer.start();
			}
			else {
				videoTimer.stop();
		...
		public function get toggleLiquidVideo() : Boolean
		{
			return _showingLiquidVideo;
		}
